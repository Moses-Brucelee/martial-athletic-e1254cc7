...
export async function fetchHeatJudges(competitionId: string): Promise<HeatJudge[]> {
  ...
  const { data, error } = await (supabase as any)
    .from("heat_judges")
    .select("id, heat_id, judge_id, lane_number, competition_judges!heat_judges_judge_id_fkey(display_name, user_id)");
...
export async function assignHeatJudge(
  heatId: string,
  judgeId: string,
  ...
  const { data, error } = await (supabase as any)
    .from("heat_judges")
    .insert({ heat_id: heatId, judge_id: judgeId, lane_number: laneNumber ?? null })
    .select()
    .single();
  if (error) throw error;
