...
import { useWorkouts, useSaveWorkouts } from "@/modules/tournaments/hooks";
...
export default function CompetitionWorkouts() {
  ...
  const { data: dbWorkouts, isLoading } = useWorkouts(id);
  const saveMutation = useSaveWorkouts();
  ...
  if (dbWorkouts && dbWorkouts.length > 0 && !initialized) {
    ...
    setInitialized(true);
  } else if (dbWorkouts && dbWorkouts.length === 0 && !initialized) {
    setInitialized(true);
  }
  ...
  const handleSave = async () => {
    ...
    try {
      await saveMutation.mutateAsync({
        competitionId: id,
        workouts: workouts.map((w, i) => ({ workout_number: i + 1, measurement_type: w.measurement_type })),
      });
    ...
    <div className="min-h-dvh bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-8">
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          {workouts.map((workout, index) => (
            <div key={index} className="flex items-center gap-3 p-3 rounded-lg bg-background border border-border">
              ...
              <Select value={workout.measurement_type} onValueChange={(val) => updateMeasurement(index, val)}>
                <SelectTrigger className="h-10 bg-background flex-1"><SelectValue placeholder="Select measurement" /></SelectTrigger>
                <SelectContent>
        ...
        <div className="flex justify-between mt-8">
          <Button variant="outline" onClick={() => navigate(-1)} disabled={saveMutation.isPending}>
